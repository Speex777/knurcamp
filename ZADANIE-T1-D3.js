...
function zakupy(balansPortfela, towary) {
    ...
    // MIEJSCE NA TWÓJ KOD
    //console.log(balansPortfela) // ilość $

    max_cena = 0
    aktualna_cena= 0
    aktualna_nazwa = ""
    ilosc_produktow_nazwa = 0
    index = -1
    x = towary.length *4

    while(balansPortfela != 0)
    {
      for(const produkt of towary)
      { 
        for(const klucz in produkt)
        {
          const rzecz_nazwa = produkt["nazwa"]
          const rzecz_cena = produkt["koszt"]
          const rzecz_liczbaSztuk = produkt["liczbaSztuk"]
          console.log("\nNazwa produktu: " + rzecz_nazwa, "\nCena produktu: " + rzecz_cena, "\nLiczba Sztuk: " + rzecz_liczbaSztuk) // wypisuje wszystko ładnie żeby wiedzieć co i jak
          console.log("balansportfela : " + balansPortfela)
          aktualna_cena = rzecz_cena;
          aktualna_nazwa = rzecz_nazwa;
          aktualna_ilosc = rzecz_liczbaSztuk;
          if(max_cena < aktualna_cena)
          {
            max_cena = aktualna_cena;
            index += 1
            x -=1
          }
        }
      
        console.log('\nwartosc a czyli index =', index)
  
        if (index > -1) 
        {
          max_cena = aktualna_cena;
          ilosc_produktow_nazwa = Math.floor(balansPortfela / max_cena); 
          console.log('\n',aktualna_nazwa, "maksymalna ilosc produktów w balans portfela to: ",ilosc_produktow_nazwa+1)

          balansPortfela -= aktualna_cena*ilosc_produktow_nazwa;
          console.log('\nNazwa produktu:', aktualna_nazwa,'\naktualna ilość dostępnych produktów to:', ilosc_produktow_nazwa);
          console.log("\nAktualny balans portfela to:", balansPortfela, "zł.\n" )
          if(ilosc_produktow_nazwa < 1)
          {
            console.log("balansportfela : " + balansPortfela)
            console.log('Do usunięcia jest towar o indeksie:', index)
            delete towary[index].koszt
            delete towary[index].liczbaSztuk
            delete towary[index].nazwa
            console.log("Usunięto towar o indeksie:", index,"\n")
            index -= 1
            //x -= 1
            console.log('\n===========================================\n')
          }
