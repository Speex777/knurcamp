...
function dodawanko(Shih_Tzu,Kotek_Zbyszek_Bury)
{
    return Shih_Tzu + Kotek_Zbyszek_Bury;
}
...
function kserokopiarka(GGG)
{
   return {...GGG} // 2 te same obiekty ale są różne, tak się kopiuje
}
...
{
    // obiektów złożonych nie można porównywać, sprawdza tylko ich referencje nie sprawdza kluczy, trzeba sprawdzić klucze
...
{
    // obiekty sprawdzamy po kluczu nie można używać == 
    ...
    for (const strona  of ksiazka) // off zwraca klucze elementarnie
    {
        if (!poprawnaksiazka.find((s) => s.strona == strona.strona)) // jedno linijkowa funkcja
        {
...
function mors(a)
{
    console.log("kod morsa do rozwiązania\n", a) // inbzyniernia podmianka tak samo
}
